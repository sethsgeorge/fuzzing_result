...
root_directory = r'cfiles'
output_file = 'output_feature.csv' 
input_csv_file = 'program_names_for_extraction.csv'
...
# Open the output CSV file in 'a' mode
with open(output_file, 'a', newline='') as out_csv:
    ...
    # Read the input CSV file containing file paths
    with open(input_csv_file, 'r', newline='') as in_csv:
        csv_reader = csv.DictReader(in_csv)
        for row in csv_reader:
            input_file_path = row['program_name']
            if input_file_path.endswith(('.c', '.i')):  # Process both .c and .i files
                # Preprocess the file
                preprocessed_file_path = preprocess_c_file(input_file_path)
                # Process and write metrics to the output CSV file
